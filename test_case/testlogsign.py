...
class Testcase:
    ...
    def test_clicklogin(self, loginfo):
        ...
        with allure.step('8.判断弹窗文字是否为：请您同意用户条款'):
            ...
            base_page.wait(1)
        with allure.step('9.点击 已阅读并同意小米账号 用户协议 和 隐私政策'):
            base_page.click(By.XPATH, '//input[@class="ant-checkbox-input"]')
            base_page.wait(1)
        with allure.step('10.点击登录按钮'):
            base_page.click(By.XPATH, '//button[@type="submit"]')
            base_page.wait(1)
        with allure.step('11.用户名或密码不正确'):
